...
function StatColumn({ labelTop, value, labelBottom, Icon, isPlaceholder = false }) {
  return (
    <div className="flex-1 min-w-0 px-3 py-2 flex items-center">
      <div className="flex-1 min-w-0">
        ...
        {labelBottom ? <div className="text-xs text-muted-foreground mt-1">{labelBottom}</div> : null}
      </div>
...
export default function CustomerStatusCard({
  ...
  const { success, missed, total } = useMemo(() => {
    ...
    const keywordsSuccess = ['terkoneksi', 'connected', 'success', 'sukses', 'answered']
    const keywordsMissed = ['voicemail', 'missed', 'no answer', 'tidak terangkat', 'tidak jawab']
      ...
      <Card className={cn('p-3', className)}>
        <CardContent>
          <div className="flex flex-col sm:flex-row items-stretch divide-y sm:divide-y-0 sm:divide-x divide-border">
            <StatColumn labelTop="Berhasil dibungi" isPlaceholder />
            <StatColumn labelTop="Telepon berdering" isPlaceholder />
            <StatColumn labelTop="Persentase" isPlaceholder />
          </div>
        </CardContent>
      </Card>
    )
  ...
  if (error) {
    return (
      <Card className={cn('p-3', className)}>
        <CardContent>
          <div className="text-sm text-destructive">Gagal memuat data status</div>
        </CardContent>
      </Card>
    )
  }
  ...
  return (
    <Card className={cn('p-3', className)}>
      <CardContent>
        <div className="flex flex-col sm:flex-row items-stretch divide-y sm:divide-y-0 sm:divide-x divide-border">
          <StatColumn labelTop="Berhasil dibungi" value={success} labelBottom="nasabah" Icon={PhoneOutgoing} />
          <StatColumn labelTop="Telepon berdering" value={missed} labelBottom="nasabah" Icon={PhoneMissed} />
          <StatColumn labelTop="Persentase" value={`${successRate}%`} labelBottom="Keberhasilan" Icon={Coins} />
        </div>
      </CardContent>
    </Card>
  )
